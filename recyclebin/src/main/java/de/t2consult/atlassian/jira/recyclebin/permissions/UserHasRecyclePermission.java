...
@Scanned
public class UserHasRecyclePermission extends com.atlassian.jira.plugin.webfragment.conditions.AbstractProjectPermissionCondition {
    ...
    @Inject
    public UserHasRecyclePermission(@ComponentImport PermissionManager permissionManager) {
    ...
    protected boolean shouldDisplay(ApplicationUser user, JiraHelper jiraHelper, ProjectPermissionKey permissionKey) {
        return IsAllowedToRecycleIssues(user,jiraHelper.getProject(),permissionKey,permissionManager);
    }
}
